...
public class UnitCommandsTests
{
    ...
    {
        _host = new UnitCommands(
            UnitConvert.DefaultParser,
            UnitConvert.DefaultConverter);
    }
    ...
    public void ConvertUnitAsExpected()
    {
        ...
        Assert.AreEqual(
            expectedResult,
            _host.ConvertUnit("12m", "in", "cm", false));
        Assert.AreEqual(
            expectedResult,
            _host.ConvertUnit("12m", "to", "cm", false));
        Assert.AreEqual(
            expectedResult,
            _host.ConvertUnit("12m", "cm", null, false));
    }
}
