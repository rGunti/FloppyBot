...
{
    private static double AsSiSize(this double size, FileSizeUnit unit) => size * Math.Pow(1000, (int)unit);
    private static double AsSiSize(this int size, FileSizeUnit unit) => size * Math.Pow(1000, (int)unit);

    private static double AsBinarySize(this double size, FileSizeUnit unit) => size * Math.Pow(1024, (int)unit);
    private static double AsBinarySize(this int size, FileSizeUnit unit) => size * Math.Pow(1024, (int)unit);
    ...
    public static double KibiBytes(this double kibiBytes) => kibiBytes.AsBinarySize(FileSizeUnit.Kilo);
    public static double MebiBytes(this double mebiBytes) => mebiBytes.AsBinarySize(FileSizeUnit.Mega);
    public static double GibiBytes(this double gibiBytes) => gibiBytes.AsBinarySize(FileSizeUnit.Giga);
    public static double TebiBytes(this double tebiBytes) => tebiBytes.AsBinarySize(FileSizeUnit.Tera);
}
