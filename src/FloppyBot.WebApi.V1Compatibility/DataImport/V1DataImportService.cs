...
{
    private static readonly IImmutableSet<string> SupportedVersions = ImmutableHashSet.Create<string>()
        .Add("2022.12")
        .Add("2022.12.0");
    ...
    public bool ProcessPayload(DataExport payload, string? userId, bool simulate)
    {
        ...
        {
            _logger.LogWarning("Running without authentication, I hope you know what this means ...");
        }

        _logger.LogDebug("Beginning pre-process phase, all calls are being created but none executed ...");
    ...
    private void ImportFiles(IImmutableList<Tuple<string, string, string, Stream>> filesToImport)
    {
        ...
        {
            _logger.LogDebug("Importing file {FileOwner}/{FileName} ({FileMimeType})",
                file.Item1,
                file.Item2,
                file.Item3);
    ...
    private void ImportQuotes(IImmutableList<Quote> quotes)
    {
        ...
        {
            _logger.LogDebug("Importing quote {QuoteChannel}/{QuoteId}",
    ...
    {
        _logger.LogDebug("Importing {CustomCommandCount} custom commands ...", customCommands.Count);
        foreach (var commandDescription in customCommands)
        {
            _logger.LogTrace("Importing custom command {@CommandChannel}/{CommandName} ...",
    ...
    private void ImportShoutoutMessages(IImmutableList<Tuple<string, string>> importShoutoutMessageSettings)
    {
        _logger.LogDebug("Importing {ShoutoutConfigCount} shoutout configurations ...",
    ...
    {
        _logger.LogDebug(
            "Preparing import of {FileCount} files ...",
            files.Count);
        ...
        var channelsWithFailedClaims = files
            ...
            .Select(fileGroup =>
            {
                var quotaRequired = fileGroup
                    .Sum(file => file.Header.FileSize);
                var fileCount = fileGroup
                    .Count();
                return Tuple.Create(
                    fileGroup.Key,
                    _fileService.CanClaimStorage(
                        fileGroup.Key,
                        quotaRequired,
                        fileCount));
            })
        ...
        return files
            .Select(PrepareImportFile)
            .ToImmutableArray();
    }

    private Tuple<string, string, string, Stream> PrepareImportFile(DataExportFile file)
    {
        _logger.LogDebug("Preparing import of file {File} for {Channel}",
    ...
    {
        _logger.LogDebug(
            "Preparing import of {FileCount} quotes ...",
            quotes.Count);
        return quotes
            .Select(PrepareImportQuote)
            .ToImmutableArray();
    }

    private Quote PrepareImportQuote(QuoteDto quote)
    {
        _logger.LogTrace("Preparing quote {QuoteChannel}/{QuoteId}",
            quote.Channel,
            quote.QuoteId);
    ...
    {
        _logger.LogDebug(
            "Preparing import of {FileCount} sound commands ...",
            soundCommands.Count);

        return soundCommands
            .Select(cmd => PrepareImportSoundCommand(cmd, knownFiles));
    }
    ...
    private CustomCommandDescription PrepareImportSoundCommand(
        ...
        IImmutableList<Tuple<string, string, string, Stream>> knownFiles)
    {
        _logger.LogTrace("Preparing sound command {CommandChannel}/{CommandName}",
            soundCommand.ChannelId,
            soundCommand.CommandName);

        if (_customCommandService.GetCommand(soundCommand.ChannelId, soundCommand.CommandName) != null)
        {
            throw new BadRequestException(
        ...
        if (soundCommand.SoundFiles != null)
        {
            foreach (var soundFileRef in soundCommand.SoundFiles)
            {
                ...
                {
                    _logger.LogTrace("{FileName} is already stored and can be referenced", soundFileRef);
                    continue;
                }
        ...
        if (soundCommand.PayloadId != null)
        {
            _logger.LogDebug(
                ...
                soundCommand.ChannelId,
                soundCommand.CommandName);
            if (!knownFiles.Any(file =>
                    file.Item1 == soundCommand.ChannelId && file.Item2 == $"{soundCommand.CommandName}.wav"))
            {
                throw new BadRequestException(
                    $"Sound command {soundCommand.ChannelId}/{soundCommand.CommandName} references a file that is not present in the import");
            }

            return _mapper.Map<CustomCommandDescription>(soundCommand with
            {
                SoundFiles = ImmutableList.Create<string>()
    ...
    {
        _logger.LogDebug("Preparing import of {ShoutoutCount} shoutout commands",
            shoutoutConfigs.Count);
        return shoutoutConfigs
            .Select(PrepareImportShoutout)
            .ToImmutableArray();
    }
