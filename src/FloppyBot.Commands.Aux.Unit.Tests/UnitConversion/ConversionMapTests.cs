...
namespace FloppyBot.Commands.Aux.Unit.Tests.UnitConversion
{
    ...
    public class ConversionMapTests
    {
        ...
        {
            ConversionNode
                originNode = _map.GetNode(origin),
        ...
        private static readonly string[] Nodes = new[] { A, B, C, D, E, F, G, H, I, J, K, L, M, N, O, P };

        private static readonly ISet<(string a, string b)> NodeRelations = new HashSet<(string a, string b)>
        {
            (A, I),
            (B, I),
            (C, J),
