...
public class TimeCommandsTest
{
        ...
        string expectedOutput)
    {
        Assert.AreEqual(
            ...
            _host.ShowCurrentTime(new ChatUser(
                "Mock/User",
                "User",
                PrivilegeLevel.Viewer), timeZoneId));
    }

    [TestMethod]
    public void OutputsUserTimeZone()
    {
        _serviceMock.Setup(s => s.GetTimeZoneForUser(It.IsAny<string>()))
        ...
        string expectedOutput)
    {
        Assert.AreEqual(
            CommandResult.SuccessWith(expectedOutput),
            _host.ShowCurrentDecimalTime(new ChatUser(
                "Mock/User",
                "User",
                PrivilegeLevel.Viewer), timeZoneId));
    }
