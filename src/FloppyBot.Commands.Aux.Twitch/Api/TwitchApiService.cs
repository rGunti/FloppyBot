...
public class TwitchApiService : ITwitchApiService
{
    ...
    {
        GetUsersResponse? userResponse = _twitchApi.Helix.Users.GetUsersAsync(logins: new List<string>
        {
            userId
        }).Result;
        ...
        if (userResponse.Users.Any())
        {
            ...
            var channelResponse = await _twitchApi.Helix.Channels.GetChannelInformationAsync(user.Id);
