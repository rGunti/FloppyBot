...
public class BuiltInCommandListSupplier : ICommandListSupplier
{
    ...
    private readonly ConcurrentDictionary<CommandInfo, IImmutableSet<string>> _cachedSourceInterfaces;
    ...
    public IEnumerable<string> GetCommandList(CommandInstruction commandInstruction)
    {
        ...
        return GetCommandList(channelId, privilegeLevel)
    ...
    private IEnumerable<CommandInfo> GetCommandList(ChannelIdentifier channelIdentifier, PrivilegeLevel privilegeLevel)
    {
        return _commands
    ...
    private bool CanBeAccessedOnInterface(CommandInfo commandInfo, ChannelIdentifier channelIdentifier)
    {
        _logger.LogTrace(
            "Fetching allowed interfaces for {CommandName}",
            commandInfo.PrimaryCommandName);
        IImmutableSet<string> commands = _cachedSourceInterfaces
            .GetOrAdd(commandInfo, GetInterfacesForCommand);
        _logger.LogTrace(
    ...
    {
        _logger.LogDebug(
            "Calculating allowed interfaces for command {Command}",
            commandInfo);
        ...
        ImmutableHashSet<string> set = handlerInterfaces.Concat(hostInterfaces).ToImmutableHashSet();
        _logger.LogDebug(
    ...
    private bool CanBeAccessedWithPrivilegeLevel(CommandInfo commandInfo, PrivilegeLevel privilegeLevel)
    {
        _logger.LogTrace(
            "Fetching privilege level for {CommandName}",
            commandInfo.PrimaryCommandName);
        PrivilegeLevel commandLevel = _cachedPrivilegeLevels.GetOrAdd(commandInfo, GetPrivilegeLevelFromCommand);
        _logger.LogTrace(
    ...
    {
        _logger.LogDebug(
            "Calculating required privilege level for command {Command}",
            commandInfo);
        ...
        IEnumerable<PrivilegeLevel> hostLevel = commandInfo.ImplementingType
            .GetCustomAttributes<PrivilegeGuardAttribute>()
            .Select(a => a.MinLevel);
        PrivilegeLevel privilegeLevel = handlerLevel.Concat(hostLevel)
            .Distinct()
            .OrderByDescending(i => i)
            .FirstOrDefault();

        _logger.LogDebug("Calculated privilege level for {Command} is {PrivilegeLevel}",
            commandInfo, privilegeLevel);
