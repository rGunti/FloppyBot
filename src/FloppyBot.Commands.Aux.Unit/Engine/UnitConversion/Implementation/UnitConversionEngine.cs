...
using FloppyBot.Commands.Aux.Unit.Engine.UnitConversion.Conversion;
using FloppyBot.Commands.Aux.Unit.Engine.UnitConversion.Implementation.PathFinding;
...
internal class UnitConversionEngine : IUnitConversionEngine
{
    ...
    private readonly IImmutableDictionary<(string from, string to), (string, string)[]> _proxyConversions;
    ...
    public IImmutableDictionary<(string @from, string to), IUnitConversion> RegisteredConversions => _conversions;

    public bool HasDirectConversion(string from, string to) => _conversions.ContainsKey((from, to));

    public bool HasInvertedConversion(string from, string to) => _conversions.ContainsKey((to, from));
    ...
    public IUnitConversion FindConversion(string from, string to) => FindConversion(from, to, true, true, false, true);

    public bool HasProxyConversion(string from, string to) => _proxyConversions.ContainsKey((from, to));

    public bool HasInvertedProxyConversion(string from, string to) => _proxyConversions.ContainsKey((to, from));

    public IUnitConversion GetProxyConversion(string from, string to)
        => new ChainedUnitConversion(GetProxyConversionSteps(from, to, out var steps),
            $"Proxy[{string.Join('>', steps)}]");

    public IUnitConversion GetInvertedProxyConversion(string from, string to)
        => new ChainedUnitConversion(GetProxyConversionSteps(to, from, out var steps)
                .Reverse()
                .Select(i => new InvertedUnitConversion(i)),
            $"Proxy[{string.Join('>', steps.Reverse())}]");

    private IEnumerable<IUnitConversion> GetProxyConversionSteps(string from, string to, out string[] steps)
    {
        ...
        foreach (var (stepFrom, stepTo) in proxyConversion)
        {
            ...
            if (conversion == null)
                throw new InvalidOperationException($"Could not find suitable conversion from {stepFrom} to {stepTo}");
        ...
        bool useConversionMap)
    {
        ...
        if (usePartialProxyConversions && TryFindAndConstructProxyConversion(from, to, out var proxyConversion))
            return proxyConversion;

        if (useCompoundProxyConversions)
        {
            if (TryFindAndConstructCompoundProxyConversion(from, to, out var compoundProxyConversion))
            ...
            if (TryFindUnitWithSameConversion(from, out var matchingSourceUnits))
            {
                ...
                {
                    if (TryFindAndConstructCompoundProxyConversion(matchingUnit, to,
            ...
            if (TryFindUnitWithSameConversion(to, out var matchingTargetUnits))
            {
                ...
                {
                    if (TryFindAndConstructCompoundProxyConversion(from, matchingUnit,
                            out var compoundProxyConversionWithEquivalentUnit))
                        return compoundProxyConversionWithEquivalentUnit;
                }
            }
        ...
        if (useConversionMap &&
            _conversionMap.IsNodeReachable(from, to, out var path))
        {
    ...
    /// <returns></returns>
    private bool TryFindAndConstructProxyConversion(string from, string to, out IUnitConversion conversion)
    {
        conversion = null;
    ...
    private bool TryFindReverseMatchingProxyConversion(
        ...
        out (string from, string to)[] conversionKeys)
    {
        var reversedConversionKeys = _proxyConversions
            .ToImmutableDictionary(i => i.Key,
                i => i.Value
                    .Reverse()
                    .Select(s => (s.Item2, s.Item1))
                    .ToArray());
        ...
        out (string from, string to)[] conversionKeys)
    {
        conversionKeys = proxyConversions
            .Where(i =>
                i.Value.Any(p => p.Item1 == from) &&
                i.Value.Any(p => p.Item2 == to))
            .Select(i => i.Key)
            .ToArray();
        ...
        bool reverse = false)
    {
        ...
        if (!derivedChain.Any()) return false;
        ...
        var chainStr = string.Join(reverse ? '<' : '>',
            derivedChain.Select(i => i.Item1)
                .Concat(new[] { derivedChain.Last().Item2 }));
    ...
    /// <param name="conversion"></param>
    /// <returns></returns>
    private bool TryFindAndConstructCompoundProxyConversion(string from, string to, out IUnitConversion conversion)
    {
        conversion = null;
        ...
        if (conversionsForSource.Any())
        {
            ...
            {
                if (TryFindMatchingProxyConversion(sourceConversionPair.to, to, out var foreignProxyChains))
                {
                    foreach (var foreignProxyChain in foreignProxyChains)
                    {
                        if (TryConstructProxyConversion(foreignProxyChain.to, to, foreignProxyChain,
                                out var partialChain))
                        {
                            var bridgeConversion = FindSafeConversion(from, sourceConversionPair.to);
        ...
        if (conversionsForTarget.Any())
        {
            ...
            {
                if (TryFindMatchingProxyConversion(from, targetConversionPair.from, out var foreignProxyChains))
                {
                    foreach (var foreignProxyChain in foreignProxyChains)
                    {
                        if (TryConstructProxyConversion(from, foreignProxyChain.from, foreignProxyChain,
                                out var partialChain))
                        {
                            var bridgeConversion = FindSafeConversion(targetConversionPair.from, to);
                            if (bridgeConversion == null)
                                continue;
